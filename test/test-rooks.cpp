...
TEST_CASE("Test generateRookBlockerMask") {
    SECTION("For Rook on D4") {
        ...
        bitboard_t blockerMask = RookLookup::generateRookBlockerMask(Square::D4);
    ...
    SECTION("For Rook on A1 - Different behaviour at corners") {
        ...
        bitboard_t blockerMask = RookLookup::generateRookBlockerMask(Square::A1);
        CHECK(correctBlocker == blockerMask);
    }
    SECTION("For Rook on A8") {
        ...
        bitboard_t blockerMask = RookLookup::generateRookBlockerMask(Square::A8);
        CHECK(correctBlocker == blockerMask);
    }
    SECTION("For Rook on H1") {
        ...
        bitboard_t blockerMask = RookLookup::generateRookBlockerMask(Square::H1);
        CHECK(correctBlocker == blockerMask);
    }
    SECTION("For Rook on H8") {
        ...
        bitboard_t blockerMask = RookLookup::generateRookBlockerMask(Square::H8);
        CHECK(correctBlocker == blockerMask);
    }
    SECTION("For Rook on A7") {
        ...
        bitboard_t blockerMask = RookLookup::generateRookBlockerMask(Square::A7);
        CHECK(correctBlocker == blockerMask);
    }
}
...
TEST_CASE("Test generation of BlockerBoards") {
    ...
    SECTION("Checks for normal squares") {
        // E5
        size_t numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::E5)).size();
        CHECK(numBoards == 1024);
        // B2
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::B2)).size();
        CHECK(numBoards == 1024);
    }
    SECTION("Checks for edge cases") {
        // A2
        size_t numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::A2)).size();
        CHECK(numBoards == 2048);
        // H7
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::H7)).size();
        CHECK(numBoards == 2048);
        // B8
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::B8)).size();
        CHECK(numBoards == 2048);
        // F1
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::F1)).size();
        CHECK(numBoards == 2048);
    }
    SECTION("Checks for corner cases") {
        // A1
        size_t numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::A1)).size();
        CHECK(numBoards == 4096);
        // H1
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::H1)).size();
        CHECK(numBoards == 4096);
        // A8
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::A8)).size();
        CHECK(numBoards == 4096);
        // H8
        numBoards = RookLookup::generateBlockerBoards(
            RookLookup::generateRookBlockerMask(Square::H8)).size();
        CHECK(numBoards == 4096);
    }
